...
export class SguAuthService {
	...
	// Hàm gen ua API SGU
	private getUa(endpoint: string | undefined): string {
		class Encrypt {
			rk(index: number): number[] {
			...
			ec(input: string, key: number): number[] {
			...
			extendKey(key: number[], length: number): number[] {
				let extended: number[] = [];
		...
		class Encryptor {
			...
			mc(str: string, length: number, offset: number): string {
			...
			isapi(endpoint: string | undefined): string {
		...
		return encryptor.gc(encryptor.isapi(endpoint));
	}
	...
	async loginToSgu(username: string, password: string) {
		try {
			...
			const { CURRENT_USER, CURRENT_USER_INFO } = await this.loginFromSGU(username, password, account ? true : false);
	...
	private async performSguLogin(username: string, password: string) {
		try {
			const ua = this.getUa(SGU_API_URL);
			const response = await axios.post(SGU_API_URL, {
				username,
				password,
				grant_type: 'password',
	...
	private async fetchStudentInfo(accessToken: string) {
		try {
			const ua = this.getUa(SGU_INFO_API_URL);
	...
	async getImageAccount(access_token: string, ma_sv: string) {
		try {
			const ua = this.getUa(`${SGU_IMAGE_ACCOUNT_API_URL}?MaSV=${ma_sv}`);
	...
	//Lấy điểm theo access token
	async getScoreFromSGU(access_token: string) {
		try {
			const ua = this.getUa(`${SGU_DIEM_API_URL}?hien_thi_mon_theo_hkdk=false`);
	...
	//get info
	async loginFromSGU(username: string, password: string, isExistAccount: boolean) {
		...
		// Gọi API Login
		const CURRENT_USER = await this.performSguLogin(username, password);

		// Gọi API lấy thông tin user
		const CURRENT_USER_INFO = await this.fetchStudentInfo(CURRENT_USER.access_token);

		if (CURRENT_USER && typeof CURRENT_USER === 'object' && CURRENT_USER.roles === "SINHVIEN") {
			// Lấy điểm
			const scoreData = await this.getScoreFromSGU(CURRENT_USER.access_token);
			this.ScoreOfUser = scoreData; // Lưu điểm của người dùng
			this.GPAOfUser = scoreData.ds_diem_hocky.find((item: any) => item.loai_nganh).dtb_tich_luy_he_4;

			// Lấy ảnh
			const imageData = await this.getImageAccount(CURRENT_USER.access_token, CURRENT_USER.userName);
			this.ImageOfUser = imageData; // Lưu ảnh của người dùng
		}


		//#region XỬ LÝ LOGIN BẰNG PUPPETEER
		...
		// }
		//#endregion
		return { CURRENT_USER: CURRENT_USER, CURRENT_USER_INFO: CURRENT_USER_INFO }
	}
}