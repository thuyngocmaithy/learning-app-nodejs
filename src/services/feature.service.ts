...
export class FeatureService {
	...
	async checkRelatedData(featureIds: string[]): Promise<{ success: boolean; message?: string }> {
		const count = await this.permissionFeatureRepository.count({
			where: { feature: { featureId: In(featureIds) } },
		});

		if (count > 0) {
			return {
				success: false,
				message: `Chức năng đang được sử dụng trong dữ liệu phân quyền. Bạn có chắc chắn xóa?`,
			};
		}
