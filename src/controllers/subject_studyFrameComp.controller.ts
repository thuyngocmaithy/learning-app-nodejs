...
export class Subject_StudyFrameCompController {
	...
	public saveSemestersForSubjects = async (req: Request, res: Response): Promise<void> => {
		try {
			const data = req.body;
			const result = await this.subject_studyFrameCompService.saveSemestersForSubjects(data);
			RequestHandler.sendResponse(res, StatusCodes.OK, "success", null, "Semesters saved successfully");
		} catch (error) {
